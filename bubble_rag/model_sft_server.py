            ...
            if task.process_pid:
                try:
                    if psutil.pid_exists(task.process_pid):
                        ...
                        # 如果进程创建时间早于5分钟前，可能是孤儿进程
                        if (current_time - process_create_time) > 300:  # 5分钟
                            true_orphan_tasks.append(task)
                            print(f"   - 🔍 检测到可能的孤儿进程: 任务 {task.task_id}, PID {task.process_pid}, 已运行 {(current_time - process_create_time)/60:.1f} 分钟")
                        else:
                            print(f"   - [OK] 正常运行的进程: 任务 {task.task_id}, PID {task.process_pid}, 运行时间 {(current_time - process_create_time)/60:.1f} 分钟")
                    else:
                        # 进程不存在，需要清理任务状态
                        true_orphan_tasks.append(task)
